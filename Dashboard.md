# Dashboard

This vault is a business operating system. The job is to convert notes into offers, offers into revenue, and revenue back into better systems.

---

## Current Focus
What is the one thing most likely to create proof, output, and money this week?

- [[02-Projects/Project - Local AI Transcription Service]]

### This Week Must Happen
- [ ] Process the queued Gaurav sample end-to-end and review output quality
- [ ] Run NightlyBrainstorm on live vault (first real inference run)
- [ ] Create 1 simple buyer-facing offer
- [ ] Send 3 pieces of outreach
- [ ] Log proof in [[06-Logs/2026-04-03]]

---

## Active Projects
Keep this list brutally small.

- [[02-Projects/Project - Local AI Transcription Service]]
- [[02-Projects/Project - Personal Market Layer]]
- [[02-Projects/Project - Service Arbitrage Hub]]

---

## Planned Projects
- [[02-Projects/Project - Web Worker SaaS]]
- [[02-Projects/Project - Personal Data Engine]]
- [[02-Projects/Project - Quiet Distribution Engine]]
- [[02-Projects/Project - Repackaged Service Marketplace]]
- [[02-Projects/Project - AI + Overseas Labor Pipeline]]
- [[02-Projects/Project - Whisper + FFmpeg Wrapper Kit]]

---

## Money Engine

### Live
- [[02-Projects/Project - Local AI Transcription Service]]

### Packaging
- [[02-Projects/Project - Personal Market Layer]]
- [[05-Monetization/Offer - Local AI Transcription Service]]

### Product Snapshots
- [[05-Monetization/_Revenue Product Pipeline Snapshot]]
- [[05-Monetization/_Offer Pipeline Snapshot]]
- [[05-Monetization/_Distribution Pipeline Snapshot]]
- [[04-Systems/02-Pipelines/_Browser Fulfillment Pipeline Snapshot]]

### Scaling Paths
- [[02-Projects/Project - Service Arbitrage Hub]]
- [[04-Systems/02-Pipelines/AI + Overseas Labor Pipeline]]
- [[02-Projects/Project - Web Worker SaaS]]
- [[04-Systems/03-Concepts/WebAssembly]]

---

## Core Systems

### Build
- [[04-Systems/01-Core Systems/Local AI Transcription Service]]
- [[04-Systems/01-Core Systems/Personal Market Layer]]
- [[04-Systems/01-Core Systems/Service Arbitrage Hub]]

### Learn
- [[04-Systems/01-Core Systems/Personal Data Engine]]

### Planned
- [[04-Systems/01-Core Systems/Web Worker SaaS]]
- [[04-Systems/01-Core Systems/Quiet Distribution Engine]]
- [[04-Systems/01-Core Systems/Repackaged Service Marketplace]]
- [[04-Systems/01-Core Systems/Whisper + FFmpeg Wrapper Kit]]

### Meta
- [[04-Systems/04-Meta/Meta-System]]
- [[04-Systems/04-Meta/Vault Operating System]]

---

## Pipelines
- [[04-Systems/02-Pipelines/Revenue Product Pipeline]]
- [[04-Systems/02-Pipelines/Service Delivery Pipeline]]
- [[04-Systems/02-Pipelines/Browser Fulfillment Pipeline]]
- [[04-Systems/02-Pipelines/AI + Overseas Labor Pipeline]]
- [[04-Systems/02-Pipelines/Automation-and-External-Pipeline]]
- [[04-Systems/02-Pipelines/_Active Product Pipelines]]

---

## Core Concepts
- [[04-Systems/03-Concepts/Coordination]]
- [[04-Systems/03-Concepts/Arbitrage]]
- [[04-Systems/03-Concepts/Distribution]]
- [[04-Systems/03-Concepts/Monetization]]
- [[04-Systems/03-Concepts/Infrastructure]]
- [[04-Systems/03-Concepts/Automation]]
- [[04-Systems/03-Concepts/Data]]
- [[04-Systems/03-Concepts/WebAssembly]]

---

## Research Queue
- [[03-Research/Research - Local AI Transcription Buyers]]
- [[03-Research/Research - Web Worker SaaS Prototype Scope]]
- [[02-Projects/Project - Personal Data Engine]]

---

## Logs And Reviews
- [[06-Logs/2026-04-02]]
- [[06-Logs/2026-04-03]]

---

## Immediate Bottleneck
- no validated customer yet
- no completed delivery example yet
- no feedback loop on pricing yet

---

## Execution Rules
- only work on things that produce output
- only build systems that help sales, fulfillment, or learning
- prefer manual proof before automation
- capture what works so the vault compounds

---

## Next Actions
- [ ] test Whisper locally
- [ ] package the output into a clean offer
- [ ] identify first buyer segment
- [ ] run one real fulfillment loop
- [ ] identify the first WebAssembly-backed browser compute slice worth packaging
